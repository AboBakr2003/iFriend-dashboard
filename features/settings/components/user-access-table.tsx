...
export function UserAccessTable() {
  ...
  return (
    <div className="space-y-4 mt-6">
      ...
      <EditUserCard open={isEditOpen} onOpenChange={handleEditOpenChange} user={editingUser} onUpdated={reload} />
          ...
          <CardHeader className="flex flex-col md:flex-row justify-between items-center gap-4 p-4">
            <div className="flex items-center gap-4 mb-0">
              <div className="relative w-72 m-0">
                ...
                <Input placeholder="Search" className="pr-10 rounded-lg placeholder:text-natural-text focus:outline-none" value={searchValue} onChange={(e) => setSearchValue(e.target.value)} />
              </div>
              <Popover>
            ...
            {selectedIds.length > 1 && (
              <Button variant="default" className={`bg-danger/10 text-danger hover:bg-danger hover:text-white px-4 py-5 mb-0`} onClick={handleBulkDeleteOpen}>
                  ...
                  <TableRow key={user.id} className="border-b last:border-0">
                    <TableCell>
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => toggleSelect(user.id, Boolean(checked))}
                      />
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        <Avatar>
                          ...
                          <AvatarFallback>{(user.name || "").split(" ").map((p) => p[0]).slice(0,2).join("").toUpperCase()}</AvatarFallback>
                        </Avatar>
                    ...
                    <TableCell className="text-right py-0">
                      <div className="flex items-center justify-end gap-2">
                        <Button variant="ghost" className="text-blue-600 hover:text-primary-blue hover:bg-primary-blue/10" onClick={() => handleEditClick(user)}>
