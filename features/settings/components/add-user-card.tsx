        ...
        <div className="flex flex-col space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col items-center space-y-2">
              <input id="user-avatar" type="file" accept="image/*" className="hidden" onChange={(e) => setAvatarFile(e.target.files?.[0])} />
              <label htmlFor="user-avatar" className="w-20 h-20 rounded-full bg-natural flex items-center justify-center cursor-pointer hover:bg-gray-300 transition-colors">
                <CameraIcon className="w-8 h-8 text-natural-text" />
              </label>
              <span className="text-sm text-natural-text">{avatarFile ? avatarFile.name : "Add a personal photo"}</span>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="user-name" placeholder="Name" className="bg-natural focus-visible:ring-blue-500" value={name} onChange={(e) => setName(e.target.value)} />
            </div>
                ...
                <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-2 z-[99999]" align="start">
                  <div className="space-y-1">
                    {(roles.length ? roles.map(r => ({ id: r.id, name: r.name })) : roleOptionsFallback.map((name, idx) => ({ id: String(idx), name }))).map((item) => (
                      <button key={item.id} onClick={() => setSelectedRoleId(item.id)} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 rounded-md transition-colors">
                        {item.name}
                      </button>
                    ))}
                  </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="user-email" type="email" placeholder="example@gmail.com" className="bg-natural focus-visible:ring-blue-500" value={email} onChange={(e) => setEmail(e.target.value)} />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input id="user-password" type="password" placeholder="Password" className="bg-natural focus-visible:ring-blue-500" value={password} onChange={(e) => setPassword(e.target.value)} />
            </div>
          </div>
